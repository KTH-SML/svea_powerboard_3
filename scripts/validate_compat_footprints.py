...
root = Path('/Users/nils/Library/CloudStorage/OneDrive-KTH/Work/ITRL/svea_powerboard_rev3/hardware/kicad')
fp_table = (root / 'fp-lib-table').read_text(errors='ignore')
lib_uri = {
    name: uri
    for name, uri in re.findall(r'\(lib \(name "([^"]+)"\)\(type "KiCad"\)\(uri "([^"]+)"\)', fp_table)
}
